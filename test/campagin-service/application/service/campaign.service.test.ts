...
describe('CampaignService 테스트', () => {
  ...
  describe('create 캠페인', () => {
    ...
    beforeEach(() => {
      ...
      mockProduct = createPartialMock<Product>({
        id: 1001,

      });


      mockCampaignType = createPartialMock<CampaignType>({
        id: 201,
        code: testCampaignDto.campaignTypeCode,
      });


      mockPlatform = createPartialMock<InfluencerPlatform>({
        id: 301,
        code: testCampaignDto.influencerPlatformCode,
      });


      mockSavedCampaign = createPartialMock<Campaign>({
        id: 1,
      ...
      mockValidScheduleTypes = testCampaignDto.schedules.map((s, i) =>
        createPartialMock<ScheduleType>({
          id: 401 + i,
          code: s.scheduleTypeCode,
          campaignTypeId: mockCampaignType.id,
          influencerPlatformId: mockPlatform.id,
        })
      );
      ...
      const campaignToSave = Campaign.create({
        name: testCampaignDto.name,
        budget: testCampaignDto.budget,
        peopleCount: testCampaignDto.peopleCount,
        productId: mockProduct.id,
      ...
      campaignDomainService.createCampaign.mockReturnValue(campaignToSave);
      ...
      const schedulesToSave = testCampaignDto.schedules.map((dto, index) =>
        CampaignSchedule.create({
          campaignId: mockSavedCampaign.id,
          scheduleTypeId: mockValidScheduleTypes[index].id,
      ...
      schedulesToSave.forEach(schedule => {
        campaignDomainService.createCampaignSchedule.mockReturnValueOnce(schedule);
      });



      campaignScheduleRepository.saveMany.mockResolvedValue(schedulesToSave as any);
    ...
    it('캠페인 생성 요청 시, 관련 정보(상품, 스케줄)를 포함하여 성공적으로 생성하고 저장된 캠페인을 반환해야 한다', async () => {
      ...
      expect(campaignScheduleRepository.saveMany).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({
            campaignId: mockSavedCampaign.id,
            scheduleTypeId: mockValidScheduleTypes[0].id,
          })
        ])
      );


      expect(scheduleTypeRepository.findByCampaignTypeAndPlatform).toHaveBeenCalledTimes(1);
      expect(campaignDomainService.validateSupportedScheduleTypes).toHaveBeenCalledTimes(1);
      expect(campaignDomainService.validateRequiredScheduleTypes).toHaveBeenCalledTimes(1);
